...
def equivalent_earth_model(
    ...
    ellps was added by oue for crsim
    ...
    x = s * np.sin(AZI)
    y = s * np.cos(AZI)
    #- added by oue for crsim? sphere option
    if ellps is 'sphere':
        print ellps
        z = RNG * np.sin(ELE)
        x = RNG * np.sin(AZI)
...
def geocentric_radius(lat, geod=None, verbose=False):
    ...
    #- added by oue for crsim?
    if geod is 'sphere':
        geod = pyproj.Geod(ellps=geod, datum=geod)
    #-
    a, b = geod.a, geod.b
